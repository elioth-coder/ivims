<x-layout>
    ...
    <div class="w-full">
        <main class="max-w-screen-2xl mx-auto flex">
            <x-sidebar active="Subagents" activeSub="List of Subagents" />
                    ...
                    <div class="py-3 min-h-screen">
                        ...
                        <div class="flex flex-col">
                            ...
                            <div class="relative overflow-x-auto w-full">
                                <table id="users-table" class="bg-white w-full text-sm text-left rtl:text-right">
                                    ...
                                    <tbody>
                                        @foreach ($users as $user)
                                            <tr class="group cursor-pointer">
                                                ...
                                                <td class="group-hover:bg-violet-200 px-8 py-6">{{ $user->role }}</td>
                                                <td class="group-hover:bg-violet-200 px-8 py-6">
                                                    <x-forms.form class="hidden" method="POST" verb="DELETE"
                                                        action="/subagent/{{ $user->id }}"
                                                        id="delete-subagent-{{ $user->id }}-form">
                                                        <button type="submit">
                                                            Delete
                                                        </button>
                                                    </x-forms.form>

                                                    <a href="/subagent/{{ $user->id }}/edit" title="Edit"
        ...
        <script>
            const confirmDelete = async (id) => {
                ...
                if (result.isConfirmed) {
                    document.querySelector(`#delete-subagent-${id}-form button`).click();
                }
            }

            (function() {
                setTimeout(() => {
                    if (document.getElementById("users-table") && typeof DataTable !==
                        'undefined') {
