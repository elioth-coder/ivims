                ...
                <section class="px-8">
                    ...
                    <div class="py-3 min-h-screen">
                        <div class="mx-auto max-w-full">
                            @if (session('message'))
                                <x-alerts.success id="alert-ticket_category">
                                    {{ session('message') }}
                                </x-alerts.success>
                            @endif
                        </div>
                        ...
                        <div class="flex flex-col">
                            <div class="w-full pb-5">
                                <a href="/setting/ticket_category/create"
                            ...
                            <div class="relative overflow-x-auto w-full">
                                <table id="ticket_categories-table" class="bg-white w-full text-sm text-left rtl:text-right">
                                    ...
                                    <tbody>
                                        @foreach ($ticket_categories as $ticket_category)
                                            <tr class="group cursor-pointer">
                                                <td class="group-hover:bg-violet-200 px-8 py-6">{{ $ticket_category->code }}</td>
                                                <td class="group-hover:bg-violet-200 px-8 py-6">{{ $ticket_category->name }}</td>
                                                <td class="group-hover:bg-violet-200 px-8 py-6">
                                                    <x-forms.form class="hidden" method="POST" verb="DELETE"
                                                        action="/setting/ticket_category/{{ $ticket_category->id }}"
                                                        id="delete-ticket_category-{{ $ticket_category->id }}-form">
                                                        <button type="submit">
                                                            Delete
                                                        </button>
                                                    </x-forms.form>

                                                    <a href="/setting/ticket_category/{{ $ticket_category->id }}/edit" title="Edit"
                                                    ...
                                                    <button onclick="confirmDelete({{ $ticket_category->id }})" title="Delete"
                                                        type="button"
        ...
        <script>
            const confirmDelete = async (id) => {
                let result = await Swal.fire({
                    title: "Delete this ticket category?",
                    icon: "warning",
                ...
                if (result.isConfirmed) {
                    document.querySelector(`#delete-ticket_category-${id}-form button`).click();
                }
            }

            (function() {
                setTimeout(() => {
                    if (document.getElementById("ticket_categories-table") && typeof DataTable !==
                        'undefined') {
                        const dataTable = new DataTable("#ticket_categories-table", {
                            fixedHeight: true,
                            searchable: true,
                            perPage: 5,
                        });
