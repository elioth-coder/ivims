                    ...
                    <div class="flex space-x-3 min-h-screen">
                        <div class="w-3/5 pb-6 pt-2">
                            ...
                            <x-card class="max-w-xl">
                                <x-card-header>Edit Vehicle Body Type</x-card-header>
                                <x-forms.form method="POST" action="/setting/vehicle_body_type/{{ $vehicle_body_type->id }}" verb="PATCH">
                                    @php
                                    if($errors->has('type')) {
                                        $type = old('type');
                                    } else {
                                        $type = (old('type')) ? old('type') : $vehicle_body_type->type;
                                    }
                                    @endphp
